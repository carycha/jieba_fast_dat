...
EXPECTED_IN_SYS_DICT: dict[str, tuple[str, str]] = {
    "Word 1 (System)": ("公司", "n"),
    "Word 2 (System)": ("生成式AI", "n"),
    "Word 3 (System)": ("蘋果20iphone", "n"),
}

# 期望在載入自定義字典後找到的詞彙和詞性
EXPECTED_IN_USER_DICT: dict[str, tuple[str, str]] = {
    "Word 1 (User)": ("郭台明明也", "n"),
    "Word 2 (User)": ("自定義dict才有的字", "n"),
    "Word 3 (User)": ("柳丁20orange", "n"),
}
...
def check_segmentation_results(
    ...
    # 檢查系統字典詞彙
    print("\n--- 系統字典詞彙檢查 ---")
    for key, (word, expected_flag) in expected_sys.items():
        if word in result_map:
            actual_flag = result_map[word]
            if actual_flag == expected_flag:
                print(
                    f"✅ PASS: {key}: **'{word}'/{actual_flag}** 成功找到，詞性匹配。"
                )
            else:
                print(
                    f"❌ FAIL: {key}: **'{word}'/{actual_flag}** 詞彙找到，"
                    f"但詞性不符 (預期: {expected_flag})。"
                )
                test_passed = False
        else:
    ...
    # 檢查自定義字典詞彙
    print("\n--- 自定義字典詞彙檢查 ---")
    for key, (word, expected_flag) in expected_user.items():
        if is_userdict_loaded:
            ...
            if word in result_map:
                ...
                if actual_flag == expected_flag:
                    print(
                        f"✅ PASS: {key}: **'{word}'/{actual_flag}** "
                        f"成功找到 (自定義)，詞性匹配。"
                    )
                else:
                    print(
                        f"❌ FAIL: {key}: **'{word}'/{actual_flag}** "
                        f"詞彙找到 (自定義)，但詞性不符 (預期: {expected_flag})。"
                    )
            else:
