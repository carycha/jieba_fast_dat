...
*   **TODO:**
    *   確認「較低記憶體佔用」和「快速載入」功能是否已實作：
        *   識別負責詞典載入和管理的核心 C++ 檔案。
        *   閱讀這些檔案的內容，以查找 DAT 結構和 mmap 相關的程式碼。
    *   確認「動態詞典」功能：不支持運行時動態添加用戶詞 (add_word / InsertUserWord 功能被移除)。如需更新用戶詞典，需要修改用戶詞典文件並重新運行程序（會自動檢測到變更並重建 DAT 緩存）。